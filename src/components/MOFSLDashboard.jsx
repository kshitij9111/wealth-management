...
const bl = (k,p="inside",min=30) => <LabelList dataKey={k} position={p} fill="#fff" fontSize={9} fontWeight={600} formatter={v=>Math.abs(v||0)>=min?fmt(v,0):""}/>;
...
// ═══ OVERVIEW ═══
const Overview = () => { const fy26=cA6[5], fy25=cA6[4], fy24=cA6[3]; return <div className="space-y-6">
  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
    <KPI label="FY26E Gross Rev" value={`₹${fmt(fy26.grossRev,0)} Cr`} sub={`${yoy(fy26.grossRev,fy25.grossRev)} YoY · ${gwth(calcCAGR(fy26.grossRev,fy24.grossRev,2))} 2Y CAGR`} trend="up" color={C.accent}/>
    <KPI label="FY26E Net Rev" value={`₹${fmt(fy26.netRev,0)} Cr`} sub={`${yoy(fy26.netRev,fy25.netRev)} YoY · ${gwth(calcCAGR(fy26.netRev,fy24.netRev,2))} 2Y CAGR`} trend="up" color={C.teal}/>
    <KPI label="FY26E Op PAT" value={`₹${fmt(fy26.opPAT,0)} Cr`} sub={`${yoy(fy26.opPAT,fy25.opPAT)} YoY · ${gwth(calcCAGR(fy26.opPAT,fy24.opPAT,2))} 2Y CAGR`} trend="up" color={C.green}/>
    <KPI label="FY26E Total PAT" value={`₹${fmt(fy26.totalPAT,0)} Cr`} sub={`${yoy(fy26.totalPAT,fy25.totalPAT)} YoY · incl. Treasury`} color={C.purple}/>
    <KPI label="FY25 Net Worth" value={`₹${fmt(fy25.nw,0)} Cr`} sub={`RoE ${fy25.roe}%`} color={C.orange}/>
    <KPI label="FY26E NW" value={`₹${fmt(fy26.nw,0)} Cr`} sub={`${yoy(fy26.nw,fy25.nw)} vs FY25 · ${gwth(calcCAGR(fy26.nw,fy24.nw,2))} 2Y CAGR`} trend="up" color={C.pink}/>
  </div>
  <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
    <Card><CT>Annual Revenue Trajectory (₹ Cr)</CT><CW h={260}><ComposedChart data={cA6}>{grid}<XAxis {...xFY}/><YAxis {...yL}/>{tip}{leg}<Bar dataKey="grossRev" name="Gross Revenue" fill={C.accent} opacity={0.3} radius={[3,3,0,0]}/><Bar dataKey="netRev" name="Net Revenue" fill={C.accent} radius={[3,3,0,0]}>{bl("netRev")}</Bar><Line dataKey="opPAT" name="Op PAT" stroke={C.green} strokeWidth={2.5} dot={{r:3}}/></ComposedChart></CW></Card>
    <Card><CT>Operating PAT vs Treasury PAT (₹ Cr)</CT><CW h={260}><ComposedChart data={[...cA.map(r=>({fy:r.fy, opPAT:r.opPAT, treasPAT:r.totalPAT-r.opPAT, totalPAT:r.totalPAT})), {fy:"FY26E", opPAT:cA6[5].opPAT, treasPAT:+(sq(cQ,"treasPAT",8,10)*4/3).toFixed(1), totalPAT:cA6[5].totalPAT}]}>{grid}<XAxis {...xFY}/><YAxis {...yL}/>{tip}{leg}<Bar dataKey="opPAT" name="Operating PAT" fill={C.green} radius={[3,3,0,0]}>{bl("opPAT")}</Bar><Bar dataKey="treasPAT" name="Treasury PAT" fill={C.purple} radius={[3,3,0,0]}>{bl("treasPAT")}</Bar><Line dataKey="totalPAT" name="Total PAT" stroke={C.red} strokeWidth={2} dot={{r:3}}/></ComposedChart></CW></Card>
  </div>
  <Card><CT>Annual Performance (₹ Cr) & RoE %</CT><CW h={280}><ComposedChart data={cA6}>{grid}<XAxis {...xFY}/><YAxis yAxisId="l" {...yL}/><YAxis yAxisId="r" {...yR} unit="%"/>{tip}{leg}<Bar yAxisId="l" dataKey="grossRev" name="Gross Rev" fill={C.accent} opacity={0.2} radius={[3,3,0,0]}/><Bar yAxisId="l" dataKey="opPAT" name="Op PAT" fill={C.green} radius={[3,3,0,0]}>{bl("opPAT")}</Bar><Bar yAxisId="l" dataKey="totalPAT" name="Total PAT" fill={C.purple} radius={[3,3,0,0]}>{bl("totalPAT")}</Bar><Line yAxisId="r" dataKey="roe" name="RoE %" stroke={C.red} strokeWidth={2.5} dot={{r:4}}/></ComposedChart></CW></Card>
  <Sec title="Segment PAT Composition" sub="Annual PAT by business (₹ Cr)"><Card><CW h={280}><BarChart data={segPAT6}>{grid}<XAxis {...xFY}/><YAxis {...yL}/>{tip}{leg}<Bar dataKey="wm" name="WM" fill={SEG[0]} stackId="a">{bl("wm",undefined,80)}</Bar><Bar dataKey="cm" name="Capital Mkt" fill={SEG[1]} stackId="a">{bl("cm",undefined,80)}</Bar><Bar dataKey="amc" name="AMC" fill={SEG[2]} stackId="a">{bl("amc",undefined,80)}</Bar><Bar dataKey="pwm" name="PWM" fill={SEG[3]} stackId="a">{bl("pwm",undefined,80)}</Bar><Bar dataKey="hfc" name="HFC" fill={SEG[4]} stackId="a">{bl("hfc",undefined,80)}</Bar></BarChart></CW></Card></Sec>
</div>; };
...
const WPTab = () => { const L=wpA[2],P=wpA[1]; return <div className="space-y-6">
  ...
  <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
    <Card><CT>Annual Net Revenue by Stream (₹ Cr)</CT><CW><BarChart data={wpA}>{grid}<XAxis {...xFY}/><YAxis {...yL}/>{tip}{leg}<Bar dataKey="nii" name="NII (Interest)" fill={C.accent} stackId="a">{bl("nii",undefined,100)}</Bar><Bar dataKey="brok" name="Brokerage" fill={C.green} stackId="a">{bl("brok",undefined,100)}</Bar><Bar dataKey="dist" name="Distribution" fill={C.purple} stackId="a">{bl("dist",undefined,100)}</Bar><Bar dataKey="other" name="Other / PMS" fill={C.slate} stackId="a"/></BarChart></CW></Card>
    <Card><CT>NII Split: WM vs PWM (₹ Cr)</CT><CW><BarChart data={wpA}>{grid}<XAxis {...xFY}/><YAxis {...yL}/>{tip}{leg}<Bar dataKey="wmNII" name="WM NII" fill={C.accent} stackId="a">{bl("wmNII",undefined,100)}</Bar><Bar dataKey="pwmNII" name="PWM NII" fill={C.teal} stackId="a">{bl("pwmNII",undefined,100)}</Bar></BarChart></CW></Card>
  </div>
  <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
    <Card><CT>Brokerage Split: WM vs PWM (₹ Cr)</CT><CW><BarChart data={wpA}>{grid}<XAxis {...xFY}/><YAxis {...yL}/>{tip}{leg}<Bar dataKey="wmBrok" name="WM Brokerage" fill={C.green} stackId="a">{bl("wmBrok",undefined,100)}</Bar><Bar dataKey="pwmBrok" name="PWM Brokerage" fill={C.orange} stackId="a">{bl("pwmBrok",undefined,100)}</Bar></BarChart></CW></Card>
    <Card><CT>Distribution Split: WM vs PWM (₹ Cr)</CT><CW><BarChart data={wpA}>{grid}<XAxis {...xFY}/><YAxis {...yL}/>{tip}{leg}<Bar dataKey="wmDist" name="WM Distribution" fill={C.purple} stackId="a">{bl("wmDist",undefined,100)}</Bar><Bar dataKey="pwmDist" name="PWM Distribution" fill={C.pink} stackId="a">{bl("pwmDist",undefined,100)}</Bar></BarChart></CW></Card>
  </div>
  <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
...
const CMTab = () => { const L=cmA[2],P=cmA[1]; return <div className="space-y-6">
  ...
  <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
    <Card><CT>Annual Net Revenue by Stream (₹ Cr)</CT><CW><BarChart data={cmA}>{grid}<XAxis {...xFY}/><YAxis {...yL}/>{tip}{leg}<Bar dataKey="netBroking" name="Net Broking" fill={C.accent} stackId="a">{bl("netBroking",undefined,50)}</Bar><Bar dataKey="feesIncome" name="Fees / IB" fill={C.purple} stackId="a">{bl("feesIncome",undefined,50)}</Bar><Bar dataKey="nii" name="NII" fill={C.teal} stackId="a">{bl("nii",undefined,50)}</Bar><Bar dataKey="otherOp" name="Other" fill={C.slate} stackId="a"/></BarChart></CW></Card>
